...
/// Locales shared by the generated installer wrappers and the jgrep Agent
/// plugin documentation. Keep this list aligned with the generator's
/// `LOCALES` mapping rather than accepting a partially generated tree.
const LOCALIZED_LOCALES: &[&str] = &[
    "en", "ja", "zh-CN", "ko", "es", "de", "ru", "fr", "pt-BR", "it", "ar", "hi",
];
...
fn check_docs() -> Result<(), String> {
    ...
    check_installer_and_plugin_contract()?;
...
fn check_installer_and_plugin_contract() -> Result<(), String> {
    if LOCALIZED_LOCALES.len() != 12 {
        return Err(format!(
            "localized installer contract must contain 12 locales, found {}",
            LOCALIZED_LOCALES.len()
        ));
    }

    let expected_locales = LOCALIZED_LOCALES
        .iter()
        .map(|locale| (*locale).to_owned())
        .collect::<BTreeSet<_>>();
    if expected_locales.len() != LOCALIZED_LOCALES.len() {
        return Err("localized installer contract contains duplicate locale names".to_owned());
    }

    let actual_locales = child_directory_names(Path::new("installers"))?;
    if actual_locales != expected_locales {
        return Err(format!(
            "installer locale directories differ from the 12-locale contract; expected {expected_locales:?}, found {actual_locales:?}"
        ));
    }

    let generator_path = "scripts/generate-localized-installers.py";
    let generator = read_text(generator_path)?;
    for required in [
        "LOCALES",
        "--check",
        "render_shell",
        "render_powershell",
        "Generated by scripts/generate-localized-installers.py",
    ] {
        require_contains(generator_path, &generator, required)?;
    }

    for locale in LOCALIZED_LOCALES {
        require_contains(generator_path, &generator, &format!("\"{locale}\""))?;
        check_localized_installer_wrapper(locale, "install.sh", "scripts/install.sh")?;
        check_localized_installer_wrapper(locale, "install.ps1", "scripts/install.ps1")?;
    }
    check_generated_installer_wrappers()?;

    let packager_path = "scripts/release/package-installers.ps1";
    let packager = read_text(packager_path)?;
    for required in [
        "Copy-RequiredDirectory -Source (Join-Path $repositoryRoot \"installers\")",
        "Copy-RequiredDirectory -Source (Join-Path $repositoryRoot \"plugins\")",
        "Copy-RequiredDirectory -Source (Join-Path $repositoryRoot \".agents\")",
        "Copy-RequiredDirectory -Source (Join-Path $repositoryRoot \"eval\")",
        "Copy-RequiredDirectory -Source (Join-Path $repositoryRoot \"templates\")",
        "generate-localized-installers.py",
        "$bundleRoot.tar.gz",
        "$bundleRoot.zip",
        "Write-Checksum",
    ] {
        require_contains(packager_path, &packager, required)?;
    }

    check_marketplace_manifest()?;
    check_plugin_manifests()?;
    check_plugin_skill_and_readmes(&expected_locales)?;
    Ok(())
}

fn check_generated_installer_wrappers() -> Result<(), String> {
    for interpreter in ["python3", "python"] {
        match Command::new(interpreter)
            .args(["-B", "scripts/generate-localized-installers.py", "--check"])
            .status()
        {
            Ok(status) if status.success() => return Ok(()),
            Ok(status) => {
                return Err(format!(
                    "generated localized installers are stale or invalid ({interpreter} exited with {status}); run scripts/generate-localized-installers.py"
                ));
            }
            Err(error) if error.kind() == ErrorKind::NotFound => continue,
            Err(error) => {
                return Err(format!(
                    "could not run {interpreter} to verify generated localized installers: {error}"
                ));
            }
        }
    }
    Err(
        "could not verify generated localized installers because neither python3 nor python is available"
            .to_owned(),
    )
}

fn child_directory_names(directory: &Path) -> Result<BTreeSet<String>, String> {
...
fn check_localized_installer_wrapper(
    locale: &str,
    filename: &str,
    core_installer: &str,
) -> Result<(), String> {
    let path = format!("installers/{locale}/{filename}");
    ...
    for required in [
        ...
        require_contains(&path, &contents, required)?;
    }
    Ok(())
}

fn parse_json(path: &str) -> Result<serde_json::Value, String> {
    let contents = read_text(path)?;
    serde_json::from_str(&contents).map_err(|error| format!("could not parse {path}: {error}"))
}
...
fn require_json_string(
    ...
    match value.pointer(pointer).and_then(serde_json::Value::as_str) {
        ...
        Some(actual) => Err(format!(
            "{path} has {pointer} = {actual:?}; expected {expected:?}"
        )),
        None => Err(format!("{path} is missing string field {pointer}")),
    }
}
...
fn check_marketplace_manifest() -> Result<(), String> {
    ...
    let plugins = manifest
        ...
        .ok_or_else(|| format!("{path} is missing plugins array"))?;
    if plugins.len() != 1 {
        return Err(format!(
            "{path} must expose exactly one plugin, found {}",
            plugins.len()
        ));
    }
    let plugin = &plugins[0];
    require_json_string(path, plugin, "/name", "jgrep-agent")?;
    require_json_string(path, plugin, "/source/source", "local")?;
    require_json_string(path, plugin, "/source/path", "./plugins/jgrep-agent")?;
    require_json_string(path, plugin, "/policy/installation", "AVAILABLE")?;
    require_json_string(path, plugin, "/policy/authentication", "ON_INSTALL")?;
    require_json_string(path, plugin, "/category", "Productivity")
}

fn check_plugin_manifests() -> Result<(), String> {
    let codex_path = "plugins/jgrep-agent/.codex-plugin/plugin.json";
    let codex_manifest = parse_json(codex_path)?;
    for (pointer, expected) in [
        ("/name", "jgrep-agent"),
        ("/version", RELEASE_VERSION),
        ("/license", "GPL-3.0-or-later"),
        ("/skills", "./skills/"),
        ("/interface/displayName", "jgrep Agent"),
        ("/interface/category", "Productivity"),
    ] {
        require_json_string(codex_path, &codex_manifest, pointer, expected)?;
    }

    let generic_path = "plugins/jgrep-agent/plugin.json";
    let generic_manifest = parse_json(generic_path)?;
    for (pointer, expected) in [
        (
            "/$schema",
        ...
        require_json_string(generic_path, &generic_manifest, pointer, expected)?;
    }
    Ok(())
}

fn check_plugin_skill_and_readmes(expected_locales: &BTreeSet<String>) -> Result<(), String> {
    let skill_path = "plugins/jgrep-agent/skills/jgrep-agent/SKILL.md";
    let skill = read_text(skill_path)?;
    for required in [
        "name: jgrep-agent",
        "jgrep --ai",
        "--ai-max-results",
        ...
        require_contains(skill_path, &skill, required)?;
    }

    let plugin_directory = Path::new("plugins/jgrep-agent");
    let actual_readmes = fs::read_dir(plugin_directory)
        .map_err(|error| format!("could not list {}: {error}", plugin_directory.display()))?
        .filter_map(Result::ok)
        .filter_map(|entry| {
            let path = entry.path();
            let name = entry.file_name().to_string_lossy().into_owned();
            (path.is_file() && name.starts_with("README") && name.ends_with(".md")).then_some(name)
        })
        .collect::<BTreeSet<_>>();
    let expected_readmes = LOCALIZED_LOCALES
        .iter()
        .map(|locale| {
            if *locale == "en" {
                "README.md".to_owned()
            } else {
                format!("README.{locale}.md")
            }
        })
        .collect::<BTreeSet<_>>();
    if actual_readmes != expected_readmes {
        return Err(format!(
            "plugin README files differ from the 12-locale contract; expected {expected_readmes:?}, found {actual_readmes:?}"
        ));
    }
    if expected_locales.len() != LOCALIZED_LOCALES.len() {
        return Err("plugin README locale contract is inconsistent with installers".to_owned());
    }

    for filename in expected_readmes {
        let path = plugin_directory.join(&filename);
        ...
        for required in [
            ...
            require_contains(&path_label, &contents, required)?;
        }
    }
    Ok(())
}
...
fn markdown_documents() -> Result<Vec<PathBuf>, String> {
    ...
    for directory in [Path::new("docs"), Path::new("eval"), Path::new("plugins")] {
