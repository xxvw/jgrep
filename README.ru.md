...
Релиз `v0.1.1` вышел до переименования репозитория, поэтому неизменяемые имена
его архива и каталога установщика по-прежнему начинаются с `localjev-grep`.
URL репозитория, текущий исходный код и команда используют имя `jgrep`.
...
## Бенчмарк токенов Codex

В контролируемом бенчмарке из трёх задач поиска позиций в коде
`jgrep --ai -F` использовал **на 16,4% меньше токенов Codex**, чем `rg -F`.
Число некэшированных входных токенов снизилось на 54,9%, а объём вывода
инструмента поиска — на 97,1%. Это один повтор, а не общая гарантия. Для двух
задач позиции совпали; в широкой задаче ответ Codex в ветви `rg` пропустил
позицию, которая присутствовала в необработанном выводе инструмента.

См. [методику и ограничения](benchmark/README.md),
[сводку результатов](benchmark/RESULTS-2026-09-22.md),
[данные каждого запуска](benchmark/results/2026-09-22.json) и
[необработанные журналы выполнения](benchmark/logs/2026-09-22/README.md).
