...
`v0.1.1` はリポジトリのリネーム前に公開されたため、変更できないアーカイブ名と
インストーラーディレクトリ名には引き続き `localjev-grep` が含まれます。
リポジトリ URL、現在のソース、インストールされるコマンド名は `jgrep` です。
...
3 種類のコード位置検索を行う条件統制ベンチマークでは、`jgrep --ai -F` の
Codex 総トークン使用量は `rg -F` より **16.4% 少なく**なりました。
非キャッシュ入力トークンは 54.9%、検索ツールの出力文字数は 97.1% 減少しました。
これは 1 回の反復結果であり、一般的な性能保証ではありません。2 タスクでは位置が
一致しましたが、広範なタスクでは `rg` 側の Codex 応答が生のツール出力に存在した
1 位置を読み飛ばしました。

[測定方法と制約](benchmark/README.md)、[結果概要](benchmark/RESULTS-2026-09-22.md)、
[各実行のデータ](benchmark/results/2026-09-22.json)、
[未加工の実行ログ](benchmark/logs/2026-09-22/README.md)を参照してください。
